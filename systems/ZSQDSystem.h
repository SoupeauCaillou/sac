...
struct ZSQDComponent {
    ZSQDComponent()
        : currentSpeed(0.f), maxSpeed(1.f), frictionCoeff(0.1f),
          newDirectionCoeff(0.01f), lateralMove(true),
          rotateToFaceDirection(false), rotationSpeed(5),
          rotationSpeedStopped(10), collideWith(0) {}

    // new directions are added in this list
    std::list<glm::vec2> directions;

    // use this when you are using a vector, eg not dependent from the point
    // position
    void addDirectionVector(glm::vec2 v) { directions.push_front(v); }
    // use this when the position vector is the target point
    void addDirectionPoint(Entity e, glm::vec2 pos) {
        directions.push_front(pos - TRANSFORM(e)->position);
    }
    ...
    // how much the new direction will influences the direction (0: don't
    // influence at all, 1: it's the new direction)
    // in range [0, 1]
