...
template <typename T> struct TypedMorphElement : public MorphElement {
    TypedMorphElement(T* _output, const T& _from, const T& _to)
        : output(_output), from(_from), to(_to) {}

    T* output;
    T from, to;
    void lerp(float t) { *output = to* t + from*(1 - t); }
...
struct MorphingComponent {
    MorphingComponent()
        : active(false), value(0), activationTime(0), timing(0) {}
    bool active;
