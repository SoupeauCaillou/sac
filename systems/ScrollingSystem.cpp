...
ScrollingSystem::ScrollingSystem() : ComponentSystemImpl<ScrollingComponent>(HASH("Scrolling", 0x9fa1c26a)) {
    /* nothing saved */
...
void ScrollingSystem::initScrolling(Entity e, ScrollingComponent* sc) {
    ...
    for (int i=0; i<2; i++) {
        se.e[i] = theEntityManager.CreateEntity(HASH("scroll/e", 0xb99f8397));
