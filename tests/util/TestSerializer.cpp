...
TEST (DefaultPropertyStruct)
{
    ...
    Property<uint16_t> p(0, OFFSET(s1, myStruct));
...
TEST (MapPropertyStringKey)
{
    ...
    MapProperty<std::string, float> p(0, 0);
    uint8_t buf[256];
    CHECK(p.size(&v) <= 256);
    p.serialize(buf, &v);
    p.deserialize(buf, &w);
...
TEST (StructSerializer)
{
    ...
    Serializer s;
    s.add(new Property<int>(0, OFFSET(a, test1)));
    s.add(new Property<float>(0, OFFSET(b, test1), 0.1));
    s.add(new StringProperty(0, OFFSET(c, test1)));
...
TEST (TestVector2)
{
    ...
    Property<glm::vec2> vp(0, 0, glm::vec2(0.001, 0));
    vp.serialize(buf, &i);
