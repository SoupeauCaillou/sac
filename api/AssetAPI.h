...
class AssetAPI {
    ...
    // get the list of filenames in directory containing "extension" in their
    // name
    virtual std::list<std::string>
    listContent(const std::string& directory,
                const std::string& extension,
                const std::string& subfolder = "") = 0;
    // get the list of filenames in assets directory containing "extension" in
    // their name
    virtual std::list<std::string>
    listAssetContent(const std::string& extension,
                     const std::string& subfolder = "") = 0;

    virtual void createDirectory(const std::string& fullpath,
                                 int permission) = 0;
...
struct FileBufferWithCursor : FileBuffer {
    FileBufferWithCursor(const FileBuffer& fb, bool pDeleteThisOnClose = false)
        : cursor(0) {
        data = fb.data;
        size = fb.size;
        deleteThisOnClose = pDeleteThisOnClose;
    }
    FileBufferWithCursor()
        : FileBuffer(), cursor(0), deleteThisOnClose(false) {}
    long int cursor;
    bool deleteThisOnClose;

    static size_t
    read_func(void* ptr, size_t size, size_t nmemb, void* datasource) {
        FileBufferWithCursor* src =
            static_cast<FileBufferWithCursor*>(datasource);
        size_t r = 0;
    ...
    static int seek_func(void* datasource, int64_t offset, int whence) {
        FileBufferWithCursor* src =
            static_cast<FileBufferWithCursor*>(datasource);
        switch (whence) {
        case SEEK_SET: src->cursor = offset; break;
        case SEEK_CUR: src->cursor += offset; break;
        case SEEK_END: src->cursor = src->size - offset; break;
        }
        return 0;
    }
