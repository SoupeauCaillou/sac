...
template<typename T>
class SceneState : public StateHandler<T> {
    public:
        SceneState(const char* pName, SceneEntityMode::Enum _enterMode, SceneEntityMode::Enum _exitMode) :
        ...
        virtual bool updatePreEnter(T , float dt) {
            return batch.update(dt);
        }
        ...
        virtual bool updatePreExit(T , float dt) {
            return batch.update(dt);
        }
